...
export function renderGenerator(root, params) {
  ...
  function sectionSeriesCodes() {
    ...
    return sectionWrap("③", "시리즈 코드 입력 (최대 15개, 3×5)", "high", [
      ...
      el("p", { class: "hint" }, "조회된 상품 데이터(이미지·가격·출하일)는 오른쪽 미리보기에 바로 반영됩니다 · 미결 사항 #2 확인 필요")
    ]);
  }
  ...
  async function lookupSeriesCodes() {
    ...
    // ⚠️ 실서비스 연동 지점: 시리즈 API 연동은 개발팀에서 진행합니다.
    // 연동 시 아래 한 줄만 fetchSeriesInfo(code) 호출로 바꾸면 됩니다 — 매핑 로직/필드 계약은
    // js/lib/seriesApi.js에 이미 만들어져 있고 실제 API 응답으로 테스트까지 끝난 상태입니다.
    // (import { fetchSeriesInfo } from "../lib/seriesApi.js"; 만 추가하면 바로 연결 가능)
    //
    //   const results = await Promise.all(codes.map(async code => {
    //     try { return await fetchSeriesInfo(code); }
    //     catch (e) { log(`⚠ 시리즈 코드 "${code}" 조회 실패: ${e.message}`); return { code }; }
    //   }));
    //
    // 지금은 API를 직접 호출하지 않고 code만 채워서 등록합니다 — blocks.js가 나머지 필드를
    // "연동 예정" 플레이스홀더로 표시하므로, 템플릿이 제대로 불러와지는지는 이 상태로도 확인됩니다.
    await new Promise(r => setTimeout(r, 400));
    draft.products = codes.map(code => ({ code }));
    log(`시리즈 코드 ${draft.products.length}건 등록 완료 (상품 상세 정보는 API 연동 후 채워짐)`);
    renderForm();
    renderPreview();
  }
